...
public class AncientPearTreeGenerator {
    private static final Identifier TEMPLATE = new Identifier("ancient_trees/ancient_pear_tree");

    public static void addPieces(StructureManager manager, BlockPos pos, List<StructurePiece> pieces){
        pieces.add(new AncientPearTreeGenerator.Piece(manager, TEMPLATE, pos));
    }
    ...
    public static class Piece extends SimpleStructurePiece {
        ...
        private Piece(StructureManager manager, Identifier identifier, BlockPos pos) {
            super(StructurePieceTypes.ANCIENT_PEAR_TREE, 0);
        ...
        Piece(StructureManager manager, CompoundTag tag) {
            super(StructurePieceTypes.ANCIENT_PEAR_TREE, tag);
...
//            this.pos = this.pos.add(0, yHeight - 4, 0);
        ...
        protected void handleMetadata(String metadata, BlockPos pos, WorldAccess world, Random random, BlockBox boundingBox) {
            if ("leaves_chest".equals(metadata)) {
                LootableContainerBlockEntity.setLootTable(world, random, pos.down(), LootTables.ANCIENT_PEAR_TREE_LEAVES_CHEST);
            }else if("root_chest".equals(metadata)) {
                LootableContainerBlockEntity.setLootTable(world, random, pos.down(), LootTables.ANCIENT_PEAR_TREE_ROOT_CHEST);
            }
        }
